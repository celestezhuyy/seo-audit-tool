...
TRANSLATIONS = {
    "zh": {
        ...
        "input_header": "开始深度审计",
        "input_info": "说明: v5.1 彻底修复了中英文案混杂问题，并大幅丰富了建议内容。",
        "input_label": "输入目标网址",
        ...
        "ppt_cover_sub": "Generated by AuditAI Pro v5.1",
        ...
        # --- Issues (Detailed & Rich) ---
        "lcp_issue": "LCP (最大内容绘制) 超标", 
        "lcp_desc": "LCP 时间为 {:.2f}s (目标 <2.5s)。页面主要内容加载过于缓慢。", 
        "lcp_impact": "LCP 是 Google 核心排名因素。加载缓慢会导致用户跳出率飙升，并直接降低在移动端的搜索排名。", 
        "lcp_sugg": "压缩图片体积（使用 WebP），使用 CDN 分发内容，推迟非关键 JS 执行，并预加载 LCP 关键元素。",
        
        "cls_issue": "CLS (累积布局偏移) 超标", 
        "cls_desc": "CLS 得分为 {:.3f} (目标 <0.1)。页面加载过程中元素发生意外位移，视觉稳定性差。", 
        ...
        "inp_desc": "INP 延迟为 {}ms (目标 <200ms)。用户点击按钮后，页面响应非常迟钝。", 
        ...
        "sitemap_invalid_sugg": "使用 XML 验证工具检查文件语法，确保没有未闭合的标签或非法字符。",
        ...
        "hreflang_invalid_sugg": "使用标准的 ISO 语言代码 (例如 'en-US' 而不是 'en_US')。",
        ...
        "hreflang_no_default_sugg": "添加 hreflang='x-default' 标签，指定默认的语言版本。",
        ...
        "alt_bad_quality_sugg": "使用描述性文本准确描述图片内容，包含相关的关键词。",
        ...
        "missing_jsonld_sugg": "推荐添加结构化数据类型: {}",
    ...
    "en": {
        "sidebar_title": "🔍 AuditAI Pro",
        "sidebar_caption": "Deep Audit Edition v5.1",
        ...
        "input_info": "Note: v5.1 features fully corrected localization, enhanced descriptions, and smart schema recommendations.",
        ...
        "ppt_cover_sub": "Generated by AuditAI Pro v5.0",
        ...
        # Issues (Fully English)
        "lcp_issue": "LCP (Largest Contentful Paint) Fails", 
        "lcp_desc": "LCP is {:.2f}s (Target <2.5s). This means users wait too long to see the main content of the page.", 
        "lcp_impact": "LCP is a Core Web Vital and ranking factor. Slow loading speeds significantly increase bounce rates and lower search rankings.", 
        "lcp_sugg": "Optimize image sizes (use WebP), implement a CDN, defer non-critical JavaScript, and preload the LCP element.",
        
        "cls_issue": "CLS (Cumulative Layout Shift) Fails", 
        "cls_desc": "CLS score is {:.3f} (Target <0.1). Elements on the page shift unexpectedly during loading.", 
        "cls_impact": "A Core Web Vital ranking factor. Visual instability frustrates users and can lead to accidental clicks, damaging brand reputation.", 
        "cls_sugg": "Set explicit width and height attributes for all images and videos, and avoid inserting dynamic content above the fold.",
        
        "inp_issue": "INP (Interaction to Next Paint) Fails", 
        "inp_desc": "INP is {}ms (Target <200ms). The page is unresponsive to user clicks or interactions.", 
        "inp_impact": "A new Core Web Vital. High latency makes the site feel 'broken' or sluggish, severely impacting user conversion rates.", 
        "inp_sugg": "Reduce main-thread blocking, break up Long Tasks, and optimize complex JavaScript event handlers.",
        ...
        "robots_desc": "A global blocking rule (Disallow: /) was detected.", 
        "robots_impact": "This prevents search engines from crawling your entire site, resulting in total de-indexing and zero organic traffic.", 
        ...
        "missing_jsonld_sugg": "Add JSON-LD Schema type: {}",
        
        "missing_hreflang": "No Hreflang", 
        "missing_hreflang_desc": "No language targeting tags found.", 
        "missing_hreflang_impact": "Search engines may show the wrong language version to international users.", 
...
def analyze_page(url, html_content, status_code, lang="zh", sitemap_has_hreflang=False):
    ...
    if not soup.find('script', type='application/ld+json'):
         ...
         custom_sugg = t['missing_jsonld_sugg'].format(spec_rec) # Fixed Logic
