...
# --- 2. 排序逻辑配置 ---
# 逻辑顺序：可访问性 -> 索引 -> 技术 -> 内容 -> 体验与性能 (含 CWV)
...
TRANSLATIONS = {
    "zh": {
        ...
        "input_header": "开始深度审计",
        "input_info": "说明: v4.2 将核心性能指标 (CWV) 与用户体验 (UX) 合并，提供更连贯的审计逻辑。",
        "input_label": "输入目标网址",
        ...
        "cat_image_ux": "页面体验与性能 (Page Experience & Performance)", # Merged

        # PPT Static Text
        "ppt_cover_title": "SEO 深度技术审计报告",
        "ppt_cover_sub": "Generated by AuditAI Pro v4.2",
        ...
        "lcp_desc": "LCP 时间为 {:.2f}s (差于 2.5s)。这意味着用户打开页面后需要等待过久才能看到主要内容。",
        "lcp_impact": "它是 Google 核心排名因素。加载缓慢会导致用户跳出率飙升，排名大幅下降。",
        "lcp_sugg": "优化图片大小 (WebP)，使用 CDN，推迟非关键 JS，预加载 LCP 元素。",
        
        "cls_issue": "CLS (累积布局偏移) 超标",
        "cls_desc": "CLS 得分为 {:.3f} (差于 0.1)。页面加载时元素发生意外位移，用户体验极差。",
        "cls_impact": "直接影响排名。布局不稳定会导致用户误触广告或按钮，损害品牌信誉。",
        "cls_sugg": "为图片/视频指定宽高 (width/height)，避免顶部动态插入内容，使用 font-display: swap。",
        
        "inp_issue": "INP (交互到绘制延迟) 超标",
        "inp_desc": "INP 延迟为 {}ms (差于 200ms)。用户点击按钮后，页面响应迟钝。",
        "inp_impact": "Google 新的核心指标。高延迟会让用户觉得网站“卡顿”，严重影响转化率。",
        "inp_sugg": "减少主线程阻塞，优化长任务 (Long Tasks)，精简 JavaScript 执行逻辑。",
        ...
        "long_title_sugg": "精简标题长度，建议控制在 60 字符以内。",
        ...
        "missing_canonical_sugg": "添加 canonical 标签。",
    ...
    "en": {
        "sidebar_title": "🔍 AuditAI Pro",
        "sidebar_caption": "Deep Audit Edition v4.2",
        ...
        "input_info": "Note: v4.2 merges CWV and UX for a more logical audit flow.",
        ...
        "cat_image_ux": "Page Experience & Performance", # Merged
        ...
        "lcp_desc": "LCP is {:.2f}s (Target < 2.5s). Users wait too long to see main content.",
        "lcp_impact": "Core Ranking Factor. Slow loading increases bounce rate and hurts rankings.",
        "lcp_sugg": "Optimize images (WebP), use CDN, defer non-critical JS.",
        
        "cls_issue": "CLS (Cumulative Layout Shift) Fails",
        "cls_desc": "CLS score is {:.3f} (Target < 0.1). Elements shift unexpectedly.",
        "cls_impact": "Ranking Factor. Bad UX leads to accidental clicks and frustration.",
        "cls_sugg": "Set explicit width/height for images/videos, avoid top banners.",
        
        "inp_issue": "INP (Interaction to Next Paint) Fails",
        "inp_desc": "INP is {}ms (Target < 200ms). Page is unresponsive to clicks.",
        "inp_impact": "New Core Vital. High latency makes site feel 'broken'.",
        "inp_sugg": "Break up long tasks, optimize event handlers.",

        "no_robots": "Missing Robots.txt", "no_robots_desc": "No robots.txt found.", "no_robots_impact": "Crawlers effectively blind.", "no_robots_sugg": "Ensure robots.txt exists and is publicly accessible.",
        ...
        "no_favicon": "Missing Favicon", "no_favicon_desc": "No icon found.", "no_favicon_impact": "Lower CTR in SERP.", "no_favicon_sugg": "Configure a global Favicon for brand visibility.",
        "duplicate": "Duplicate Content", 
        "duplicate_desc": "Duplicate content not pointing to same Canonical URL.", 
        "duplicate_impact": "Keyword cannibalization & diluted link equity.",
        ...
        "cls_risk": "CLS Risk (Static)", "cls_risk_desc": "Missing dim.", "cls_risk_impact": "Layout shifts & ranking drop.",
        "missing_title": "Missing Title", "missing_title_desc": "No title.", "missing_title_impact": "Severe ranking loss.", "missing_title_sugg": "Add descriptive title.",
        "short_title": "Title Short", "short_title_desc": "Too short.", "short_title_impact": "Missed keywords.", "short_title_sugg": "Increase length.",
        "long_title": "Title Long", "long_title_desc": "Too long.", "long_title_impact": "Truncated in SERP.", "long_title_sugg": "Shorten title.",
        "missing_desc": "Missing Description", "missing_desc_desc": "No meta desc.", "missing_desc_impact": "Lower CTR.", "missing_desc_sugg": "Add description.",
        "short_desc": "Description Short", "short_desc_desc": "Too thin.", "short_desc_impact": "Less appealing.", "short_desc_sugg": "Expand description.",
        "missing_h1": "Missing H1", "missing_h1_desc": "No H1.", "missing_h1_impact": "Poor structure.", "missing_h1_sugg": "Add unique H1.",
        "missing_viewport": "No Viewport", "missing_viewport_desc": "Mobile unfriendly.", "missing_viewport_impact": "Mobile ranking drop.", "missing_viewport_sugg": "Add viewport meta.",
        "missing_canonical": "No Canonical", "missing_canonical_desc": "Missing tag.", "missing_canonical_impact": "Duplicate content risk.", "missing_canonical_sugg": "Add canonical tag.",
        ...
        "missing_hreflang": "No Hreflang", "missing_hreflang_desc": "Missing tags.", "missing_hreflang_impact": "Poor internationalization.", "missing_hreflang_sugg": "Add hreflang tags.", 
        "soft_404": "Soft 404", "soft_404_desc": "Fake 200.", "soft_404_impact": "Wasted budget.", "soft_404_sugg": "Return 404 status.",
        ...
        "url_uppercase": "URL Uppercase", "url_uppercase_desc": "Has Upper.", "url_uppercase_impact": "Duplicate risk.", "url_uppercase_sugg": "Use lowercase."
    }
}
...
def check_cwv_issues(cwv_data, url, lang="zh"):
    ...
    # Assign category to "image_ux" (Page Experience & Performance)
    category_key = "image_ux"
    ...
    if lcp > 2.5:
        ...
        issues.append({
            ...
            "severity": severity,
            "title": t["lcp_issue"],
            "desc": t["lcp_desc"].format(lcp),
            "impact": t["lcp_impact"],
            "suggestion": t["lcp_sugg"],
            "url": url,
            "examples": [url] 
        })
    ...
    if cls > 0.1:
        ...
        issues.append({
            ...
            "severity": severity,
            "title": t["cls_issue"],
            "desc": t["cls_desc"].format(cls),
            "impact": t["cls_impact"],
            "suggestion": t["cls_sugg"],
            "url": url,
            "examples": [url]
        })
    ...
    if inp > 200:
        ...
        issues.append({
            ...
            "severity": severity,
            "title": t["inp_issue"],
            "desc": t["inp_desc"].format(inp),
            "impact": t["inp_impact"],
            "suggestion": t["inp_sugg"],
            "url": url,
            "examples": [url]
        })
...
def check_site_level_assets(start_url, lang="zh", manual_robots=None, manual_sitemaps=None):
    ...
    try:
        ...
        if r.status_code != 200:
            issues.append({"category": "access", "severity": "Medium", "title": t["no_robots"], "desc": t["no_robots_desc"], "impact": t["no_robots_impact"], "suggestion": t["no_robots_sugg"], "url": robots_url, "examples": [robots_url]})
        else:
            content = r.text.lower()
            if "disallow: /" in content and "allow:" not in content:
                 issues.append({"category": "access", "severity": "Critical", "title": t["robots_bad_rule"], "desc": t["robots_desc"], "impact": t["robots_impact"], "suggestion": "Remove global disallow rule.", "url": robots_url, "examples": [robots_url]})
            if "sitemap:" not in content:
                 issues.append({"category": "access", "severity": "Low", "title": t["robots_no_sitemap"], "desc": t["robots_no_sitemap_desc"], "impact": t["robots_no_sitemap_impact"], "suggestion": "Add 'Sitemap: [URL]' directive.", "url": robots_url, "examples": [robots_url]})
        r.close()
    except: 
        issues.append({"category": "access", "severity": "Medium", "title": t["no_robots"], "desc": "Connection failed.", "impact": t["no_robots_impact"], "suggestion": "Check server config.", "url": robots_url, "examples": [robots_url]})
            ...
            if r.status_code == 200:
                try:
                    ...
                    if not sitemap_url.endswith('.gz'): 
                        issues.append({"category": "access", "severity": "Medium", "title": t["sitemap_invalid"], "desc": t["sitemap_invalid_desc"], "impact": t["sitemap_invalid_impact"], "suggestion": "Check XML syntax.", "url": sitemap_url, "examples": [sitemap_url]})
            else:
                if manual_sitemaps:
                    issues.append({"category": "access", "severity": "Low", "title": t["no_sitemap"], "desc": f"{t['no_sitemap_desc']} (Status: {r.status_code})", "impact": t["no_sitemap_impact"], "suggestion": "Check URL.", "url": sitemap_url, "examples": [sitemap_url]})
        except:
            if manual_sitemaps: issues.append({"category": "access", "severity": "Low", "title": t["no_sitemap"], "desc": "Connection failed.", "impact": t["no_sitemap_impact"], "suggestion": "Check URL.", "url": sitemap_url, "examples": [sitemap_url]})

    if not any_sitemap_valid and not manual_sitemaps:
         issues.append({"category": "access", "severity": "Low", "title": t["no_sitemap"], "desc": t["no_sitemap_desc"], "impact": t["no_sitemap_impact"], "suggestion": "Ensure sitemap.xml exists.", "url": sitemap_urls_to_check[0], "examples": [sitemap_urls_to_check[0]]})
                ...
                if final_status == 200:
                    ...
                    if current_hash in seen_hashes:
                        ...
                        if len(current_url) < len(original_url) and '?' not in current_url and '?' in original_url:
                            ...
                            if not is_handled:
                                page_issues.append({"category": "indexability", "severity": "High", "title": t["duplicate"], 
                                "desc": f"{t['duplicate_desc']} (Original: {original_url})", 
                                "impact": t["duplicate_impact"], 
                                "suggestion": t["duplicate_sugg"], "url": current_url, "meta": f"Duplicate of: {original_url}"})
                    else:
                        seen_hashes[current_hash] = current_url
...
def create_styled_pptx(slides_data, lang="zh"):
    ...
    for issue in slides_data:
        ...
        ex_box = slide.shapes.add_textbox(Inches(7), Inches(2.0), Inches(5.8), Inches(1.5)) # Reduced height
        tf = ex_box.text_frame
        tf.word_wrap = True
        tf.auto_size = MSO_AUTO_SIZE.TEXT_TO_FIT_SHAPE
        
        # Handle Duplicates in List
        if "Duplicate" in issue['title'] or "重复" in issue['title']:
            pass
        
        for idx, url in enumerate(issue['examples'][:4]): # Limit to 4
            p = tf.add_paragraph()
            p.text = f"• {url}"
        ...
        # Extract clean URL from duplicate string if needed
        if "Duplicate" in issue['title']: ex_url = ex_url.split('\n')[0]
