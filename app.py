...
TRANSLATIONS = {
    "zh": {
        ...
        "sidebar_caption": "旗舰审计版 v5.4 (Fix)",
        "nav_label": "功能导航",
        ...
        "input_header": "开始深度审计",
        "input_info": "说明: v5.4 修复了输入框变量名错误，确保爬虫能正常启动。",
        "input_label": "输入目标网址",
        ...
        "ppt_cover_sub": "Generated by AuditAI Pro v5.4",
    ...
    "en": {
        "sidebar_title": "🔍 AuditAI Pro",
        "sidebar_caption": "Deep Audit Edition v5.4",
        ...
        "input_info": "Note: v5.4 fixes input variable name issue for stable crawling.",
        ...
        # Issues
        "lcp_issue": "LCP Fails", "lcp_issue_desc": "LCP is {:.2f}s (Target <2.5s). Main content takes too long to appear.", "lcp_issue_impact": "LCP is a core ranking factor. Slow loading speeds significantly increase bounce rates and lower search rankings.", "lcp_issue_sugg": "Optimize image sizes (use WebP), implement a CDN, defer non-critical JavaScript, and preload the LCP element.",
        "cls_issue": "CLS Fails", "cls_issue_desc": "CLS score is {:.3f} (Target <0.1). Elements on the page shift unexpectedly during loading.", "cls_issue_impact": "A Core Web Vital ranking factor. Visual instability frustrates users and can lead to accidental clicks, damaging brand reputation.", "cls_issue_sugg": "Set explicit width and height attributes for all images and videos, and avoid inserting dynamic content above the fold.",
        "inp_issue": "INP Fails", "inp_issue_desc": "INP is {}ms (Target <200ms). The page is unresponsive to user clicks or interactions.", "inp_issue_impact": "A new Core Web Vital. High latency makes the site feel 'broken' or sluggish, severely impacting user conversion rates.", "inp_issue_sugg": "Reduce main-thread blocking, break up Long Tasks, and optimize complex JavaScript event handlers.",
...
def crawl_website(start_url, max_pages, lang, manual_robots, manual_sitemaps, psi_key):
    ...
    try:
        site_issues, sitemap_has_hreflang = check_site_level_assets(
            start_url, lang, manual_robots, manual_sitemaps
        )
    ...
    if psi_key:
        with st.spinner(TRANSLATIONS[lang]["psi_fetching"]):
            ...
            if cwv_data and "error" not in cwv_data: 
                ...
                issues_list = check_cwv_issues(cwv_data, start_url) # Not lang dependent at data level
    ...
    while queue and count < max_pages:
        ...
        try:
            ...
            # 1. 3xx
            if response.history:
                all_issues.append({"id": "http_3xx", "category": "access", "severity": "Medium", "url": url, "args": [current_url]})
            ...
            if 'text/html' in content_type:
                page_data, page_issues, _ = analyze_page(current_url, response.content, final_status, sitemap_has_hreflang)
...
def create_styled_pptx(slides_data, lang):
    ...
    def draw_serp_preview(slide, issue_title, evidence, url):
        box = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(7), Inches(4), Inches(5.8), Inches(2.5)) 
        box.fill.solid()
    ...
    for s in slides_data:
        ...
        p.text = t_data['title'] # Translated Title
        ...
        is_serp = any(k in s['id'] for k in ["title", "desc"])
        ...
        if is_serp: draw_serp_preview(slide, t_data['title'], ev, ex_url)
        elif is_rich: draw_rich_snippet_preview(slide, ex_url)
...
if menu_key == "input":
    ...
    if not st.session_state['audit_data']: st.warning(ui["warn_no_data"])
        ...
        # Translate IDs for chart
        id_counts = pd.DataFrame(issues)['id'].value_counts().reset_index()
        id_counts.columns = ['id', 'count']
        id_counts['name'] = id_counts['id'].apply(lambda x: get_translated_text(x, lang)['title'])
        st.bar_chart(id_counts.set_index('name'))
