...
TRANSLATIONS = {
    "zh": {
        ...
        "sidebar_caption": "旗舰审计版 v9.8 (Final)",
        "nav_label": "功能导航",
        ...
        "input_header": "开始深度审计",
        "input_info": "说明: v9.8 完美复刻 Google SERP 样式，优化 3xx 跨域显示。",
        ...
        "ppt_cover_sub": "Generated by AuditAI Pro v9.8",
...
def crawl_website(start_url, max_pages, lang, manual_robots, manual_sitemaps, psi_key, list_url=None, detail_url=None, check_robots=True, crawl_sitemap=True, allow_sub=False, allow_outside=False):
    ...
    while queue and count < max_pages:
        ...
        try:
            ...
            # 1. 3xx Chain Logging
            if response.history:
                # Build list of URLs in chain
                chain_list = [r.url for r in response.history] + [current_url]
                chain_display_parts = []
                
                # Logic: Compare domain of current node with start_netloc (origin)
                # If domain is different -> show FULL URL
                # If domain is same -> show RELATIVE path
                
                # Base is the ORIGIN of the chain
...
def draw_cwv_gauge(slide, metric, value, thresholds):
    ...
    def add_label(x_offset, text):
        tb = slide.shapes.add_textbox(Inches(x_start + x_offset - 0.2), Inches(y_pos + 0.35), Inches(0.5), Inches(0.3))
        ...
        if "favicon" in issue_id:
             ...
             if len(parts) > 1:
                 ...
                 for i, part in enumerate(display_parts):
                     # Adjust box width for potentially long URLs
                     ...
                     b.text_frame.text = part # Full path now
                     b.text_frame.paragraphs[0].font.size = Pt(9) # Smaller font for URL
    ...
    for s in slides_data:
        ...
        for idx, url in enumerate(s['examples'][:4]): 
            ...
            if s['id'] == 'duplicate' and "Duplicate Group:" in url:
                 parts = url.split("\n")
                 p.text = f"Group {idx+1}:\n   • {parts[1].replace('- ', '').strip()[:50]}...\n   • {parts[2].replace('- ', '').strip()[:50]}..."
