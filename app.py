...
TRANSLATIONS = {
    "zh": {
        ...
        "input_header": "开始深度审计",
        "input_info": "说明: v8.7 优化了重复内容展示，增加了对 Robots.txt 规则合理性的深度检查，并自动跳过登录页检查。",
        ...
        "ppt_cover_sub": "Generated by AuditAI Pro v8.7",
        ...
        "robots_quality_issue": "Robots.txt 规则配置不当", "robots_quality_issue_desc": "Robots.txt 文件存在潜在问题：{}。", "robots_quality_issue_impact": "可能导致重要资源（如CSS/JS）被屏蔽，影响Google对页面的渲染和理解。", "robots_quality_issue_sugg": "检查 Robots.txt，确保允许爬虫访问样式表和脚本文件。",
    ...
    "en": {
        "sidebar_title": "🔍 AuditAI Pro",
        "sidebar_caption": "Deep Audit Edition v8.7",
        ...
        "input_info": "Note: v8.7 features improved duplicate grouping and Robots.txt analysis.",
        ...
        try:
            ...
            if r.status_code != 200:
                ...
                if len(content.strip()) < 5:
                     issues.append({"id": "robots_quality_issue", "category": "access", "severity": "Medium", "url": robots_url, "args": ["File is empty or too short"]})
                # No User-agent check
                if "user-agent" not in content:
                     issues.append({"id": "robots_quality_issue", "category": "access", "severity": "Medium", "url": robots_url, "args": ["Missing User-agent directive"]})
                # Resource blocking check
                if "disallow: /*.css" in content or "disallow: /*.js" in content:
                     issues.append({"id": "robots_quality_issue", "category": "access", "severity": "High", "url": robots_url, "args": ["Blocking CSS/JS resources"]})
    ...
    while queue and count < max_pages:
        ...
        try:
            ...
            if response.history:
                ...
                chain_str = " -> ".join([urlparse(u).path for u in chain_list]) # Use Path only for cleaner string
...
def create_styled_pptx(slides_data, lang):
    ...
    for s in slides_data:
        ...
        for idx, url in enumerate(s['examples'][:4]): 
            ...
            if s['id'] == 'duplicate' and "Duplicate Pair:" in url:
                 parts = url.split("Duplicate Pair:")
    ...
    if not st.session_state['audit_issues']: st.warning(ui["warn_no_data"])
        ...
        for i in raw:
            ...
            if len(grouped[iid]['examples']) < 5:
                if iid == "duplicate" and "meta" in i:
                     grouped[iid]['examples'].append(f"{i['url']} Duplicate Pair: {i['meta']}")
                else:
        ...
        with st.container(border=True):
            ...
            with c2:
                ...
                if "Duplicate" in ex_url: 
                    ex_url = ex_url.split("Duplicate Pair:")[0].strip()
                if "3xx" in s['id'] and s.get('args'): 
                    ev = s['args'][0]
                ...
                for ex in s['examples']:
                     if "Duplicate Pair:" in ex:
                         parts = ex.split("Duplicate Pair:")
                         st.markdown(f"- **Group:**\n  - `{parts[0].strip()}`\n  - `{parts[1].strip()}`")
                     else:
                         st.markdown(f"- `{ex}`")
