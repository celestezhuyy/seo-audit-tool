...
TRANSLATIONS = {
    "zh": {
        ...
        "input_header": "开始深度审计",
        "input_info": "说明: v5.0 彻底修复了语言混杂问题，增强了建议的具体性和结构化数据智能推荐。",
        "input_label": "输入目标网址",
        ...
        # --- Issues (Enriched Chinese) ---
        "lcp_issue": "LCP (最大内容绘制) 超标", "lcp_desc": "LCP 为 {:.2f}s (目标 <2.5s)。这意味着用户打开页面后需要等待过久才能看到主要内容。", "lcp_impact": "LCP 是 Google 核心排名因素。加载缓慢会导致用户跳出率飙升，并直接降低搜索排名。", "lcp_sugg": "优化图片大小 (WebP)，使用 CDN 分发内容，推迟非关键 JS 执行，并预加载 LCP 关键元素。",
        "cls_issue": "CLS (累积布局偏移) 超标", "cls_desc": "CLS 为 {:.3f} (目标 <0.1)。页面加载过程中元素发生意外位移，视觉稳定性差。", "cls_impact": "作为核心排名因素，布局不稳定会导致用户误触广告或按钮，严重损害品牌信誉和用户体验。", "cls_sugg": "为所有图片和视频元素指定明确的 width 和 height 属性，避免在顶部动态插入内容。",
        "inp_issue": "INP (交互延迟) 超标", "inp_desc": "INP 为 {}ms (目标 <200ms)。用户点击按钮后，页面响应非常迟钝。", "inp_impact": "Google 新引入的核心指标。高延迟会让用户觉得网站“卡顿”或无响应，严重影响转化率。", "inp_sugg": "减少主线程阻塞，将长任务 (Long Tasks) 拆分为小任务，并优化复杂的 JavaScript 事件处理逻辑。",
        ...
        "robots_bad_rule": "Robots.txt 封禁风险", "robots_desc": "检测到全站封禁规则 (Disallow: /)，且未发现针对 Googlebot 的例外规则。", "robots_impact": "这将直接导致搜索引擎停止抓取并索引您的网站，所有自然搜索流量将归零。", "robots_bad_rule_sugg": "立即移除 'Disallow: /' 规则，或者为搜索引擎爬虫添加具体的 'Allow' 规则。",
        ...
        "4xx_title": "死链 (4xx)", "4xx_desc": "内部链接返回 404 (未找到) 或 403 (禁止访问) 错误。", "4xx_impact": "严重破坏用户体验，中断权重传递路径，并可能导致已索引的页面被 Google 移除。", "4xx_sugg": "移除死链，或者将其重定向到最相关的有效页面。",
        ...
        "cls_risk": "CLS 布局偏移风险 (静态检测)", "cls_risk_desc": "检测到 <img> 标签缺失 width 或 height 属性。", "cls_risk_impact": "图片加载时会撑开页面，导致布局发生意外抖动，直接恶化 CLS 指标。", "cls_risk_sugg": "在 HTML 中显式指定图片和视频的 width 和 height 属性。",
        ...
        "missing_jsonld": "缺失结构化数据", "missing_jsonld_desc": "未检测到 Schema.org 标记。", "missing_jsonld_impact": "错失富媒体搜索结果（Rich Results），在 SERP 中不如竞争对手显眼。", "missing_jsonld_sugg": "推荐添加结构化数据类型: {}",
    ...
    "en": {
        "sidebar_title": "🔍 AuditAI Pro",
        "sidebar_caption": "Deep Audit Edition v5.0",
        ...
        "input_info": "Note: v5.0 features fully corrected localization, enhanced descriptions, and smart schema recommendations.",
