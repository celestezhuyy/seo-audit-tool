...
TRANSLATIONS = {
    "zh": {
        ...
        "sidebar_caption": "旗舰审计版 v9.5 (Final)",
        "nav_label": "功能导航",
        ...
        "input_header": "开始深度审计",
        "input_info": "说明: v9.5 优化了 3xx 路径显示完整性，并微调了 CWV 图表标签位置。",
        ...
        "ppt_cover_sub": "Generated by AuditAI Pro v9.5",
    ...
    while queue and count < max_pages:
        ...
        try:
            ...
            if response.history:
                ...
                # Smart Path Extraction for visualization (v9.2 Fix)
                chain_parts = []
                for u in chain_list:
                    p = urlparse(u).path
                    if len(p) > 25: # Slightly more budget
                        p = p[:10] + "..." + p[-10:] # Start...End
                    if not p: p = "/"
...
def draw_cwv_gauge(slide, metric, value, thresholds):
    ...
    # Move label higher to y_pos - 0.7
    tb = slide.shapes.add_textbox(Inches(marker_x - 0.5), Inches(y_pos - 0.7), Inches(1), Inches(0.3))
    p = tb.text_frame.add_paragraph()
    p.text = f"{value}"
    p.font.size = Pt(11)
    p.font.bold = True
...
def create_styled_pptx(slides_data, lang):
    ...
    def draw_hreflang_preview(slide, url, missing_type):
        ...
        # Check if missing type contains a language code (format: "invalid: cn")
        invalid_code = None
        if "invalid" in missing_type and ":" in missing_type:
            invalid_code = missing_type.split(":")[1].strip()
        ...
        if "default" in missing_type:
             ...
             # Show the actual invalid code found
             p.text = f'<link rel="alternate" hreflang="{invalid_code}" ... /> <!-- ❌ Invalid -->'
             set_font(p.font, 11, True, RGBColor(200, 0, 0))
             
             p = tf.add_paragraph()
             p.text = '<!-- Use ISO 639-1 (e.g., "zh-CN") -->'
             set_font(p.font, 10, False, RGBColor(0, 128, 0))
        else:
             # Fallback generic
