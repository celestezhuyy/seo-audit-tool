...
# 恢复缺失的优先级列表定义
ISSUE_PRIORITY_LIST = [
    # 1. Core Web Vitals (Real Data) & Performance (Static)
    "lcp_issue", "inp_issue", "cls_issue", "cls_risk", 
    
    # 2. Access & Infrastructure
    "no_robots", "robots_bad_rule", "no_sitemap", "sitemap_invalid", "robots_no_sitemap",
    "http_5xx", "http_4xx", "http_3xx", "soft_404",
    
    # 3. Indexability & Canonicalization
...
TRANSLATIONS = {
    "zh": {
        ...
        "input_header": "开始深度审计",
        "input_info": "说明: v4.8 优化了中英文文案的专业度与丰富度，确保报告更具指导意义。",
        "input_label": "输入目标网址",
        ...
        # Categories Labels
        "cat_cwv_performance": "核心性能与体验 (Performance & UX)",
        ...
        "cat_image_ux": "资源优化 (Assets)",

        "ppt_cover_title": "SEO 深度技术审计报告",
        "ppt_cover_sub": "Generated by AuditAI Pro v4.8",
        ...
        # --- Issues (Enriched) ---
        "lcp_issue": "LCP (最大内容绘制) 超标", 
        "lcp_desc": "LCP 时间为 {:.2f}s (目标应 <2.5s)。用户打开页面后，主要内容加载过于缓慢。", 
        "lcp_impact": "LCP 是 Google 核心排名因素。加载缓慢会导致用户跳出率飙升，并直接降低搜索排名。", 
        "lcp_sugg": "压缩图片体积（使用 WebP），使用 CDN 分发内容，推迟非关键 JS 执行，并预加载 LCP 关键元素。",
        
        "cls_issue": "CLS (累积布局偏移) 超标", 
        "cls_desc": "CLS 得分为 {:.3f} (目标应 <0.1)。页面加载过程中元素发生意外位移，视觉稳定性差。", 
        "cls_impact": "作为核心排名因素，布局不稳定会导致用户误触广告或按钮，严重损害品牌信誉和用户体验。", 
        "cls_sugg": "为所有图片和视频元素指定明确的 `width` 和 `height` 属性，避免在顶部动态插入内容。",
        
        "inp_issue": "INP (交互到绘制延迟) 超标", 
        "inp_desc": "INP 延迟为 {}ms (目标应 <200ms)。用户点击按钮后，页面响应非常迟钝。", 
        "inp_impact": "Google 新引入的核心指标。高延迟会让用户觉得网站“卡顿”或无响应，严重影响转化率。", 
        "inp_sugg": "减少主线程阻塞，将长任务 (Long Tasks) 拆分为小任务，并优化复杂的 JavaScript 事件处理逻辑。",

        "no_robots": "缺失 Robots.txt", 
        "no_robots_desc": "无法访问根目录的 robots.txt 文件，或者服务器返回错误状态码。", 
        "no_robots_impact": "爬虫可能抓取无用的后台页面，不仅消耗服务器资源，还会浪费宝贵的爬取预算。", 
        "no_robots_sugg": "在网站根目录创建标准的 robots.txt 文件，并确保其对搜索引擎爬虫公开可见。",
        
        "robots_bad_rule": "Robots.txt 封禁风险", 
        "robots_desc": "检测到全站封禁规则 (Disallow: /)，且未发现针对 Googlebot 的例外规则。", 
        "robots_impact": "这将直接导致搜索引擎停止抓取并索引您的网站，所有自然搜索流量将归零。", 
        
        "robots_no_sitemap": "Robots.txt 未声明 Sitemap", 
        "robots_no_sitemap_desc": "robots.txt 文件中未指明 Sitemap XML 文件的位置。", 
        "robots_no_sitemap_impact": "会降低搜索引擎发现新页面和更新旧内容的速度，尤其对于大型网站影响更明显。", 
        
        "no_sitemap": "Sitemap 访问失败", 
        "no_sitemap_desc": "无法访问 Sitemap 文件，服务器返回 4xx 或 5xx 错误。", 
        "no_sitemap_impact": "搜索引擎难以发现深层链接或孤岛页面，导致整体收录率下降。", 
        
        "sitemap_invalid": "Sitemap 格式错误", 
        "sitemap_invalid_desc": "XML 解析失败，文件格式不符合标准协议。", 
        "sitemap_invalid_impact": "搜索引擎无法读取其中的链接，导致 Sitemap 完全失效。", 
        
        "no_favicon": "缺失 Favicon", 
        "no_favicon_desc": "未在首页检测到 Favicon 图标。", 
        "no_favicon_impact": "降低品牌在浏览器标签页和搜索结果页 (SERP) 中的辨识度，进而导致点击率 (CTR) 下降。", 
        "no_favicon_sugg": "制作一个 .ico 或 .png 格式的图标，并在 `<head>` 中通过 `<link rel='icon'>` 引用。",

        "duplicate": "发现未规范化的重复内容", 
        "duplicate_desc": "多个 URL 指向相同内容，且未通过 Canonical 标签指定唯一版本。", 
        "duplicate_impact": "导致关键词内部竞争 (Cannibalization)，分散页面权重，使所有相关页面都难以获得高排名。", 
        "duplicate_sugg": "确定一个主 URL，并在所有副本页面上添加 `rel='canonical'` 指向该主 URL。",
        
        "3xx_title": "内部链接重定向 (3xx)", 
        "3xx_desc": "内部链接指向了一个发生 301 或 302 跳转的地址。", 
        "3xx_impact": "浪费爬虫预算，增加页面加载延迟，且每次跳转都会损耗少量链接传递的权重 (Link Equity)。", 
        "3xx_sugg": "批量更新内部链接，使其直接指向最终的目标 URL，避免中间跳转。",
        
        "4xx_title": "死链/客户端错误 (4xx)", 
        "4xx_desc": "内部链接返回 404 (未找到) 或 403 (禁止访问) 错误。", 
        "4xx_impact": "严重破坏用户体验，中断权重传递路径，并可能导致已索引的页面被 Google 移除。", 
        "4xx_sugg": "移除死链，或者将其重定向到最相关的有效页面。",
        
        "5xx_title": "服务器错误 (5xx)", 
        "5xx_desc": "服务器响应 500/502/503 等内部错误。", 
        "5xx_impact": "表明服务器极其不稳定，Googlebot 会因此降低对该站点的爬取频率以减轻负载。", 
        "5xx_sugg": "检查服务器错误日志，优化数据库查询或升级服务器配置。",

        "hreflang_invalid": "Hreflang 格式错误", 
        "hreflang_invalid_desc": "语言代码不符合 ISO 639-1 标准 (如使用了错误的代码格式)。", 
        "hreflang_invalid_impact": "Google 无法识别目标语言，导致国际化定位失效，不同地区用户可能看到错误的语言页面。", 
        
        "hreflang_no_default": "Hreflang 缺失 x-default", 
        "hreflang_no_default_desc": "未配置 'x-default' 回退版本。", 
        "hreflang_no_default_impact": "当用户来自未指定的语言/地区时，可能无法自动匹配到最合适的通用版本（通常是英语）。", 
        
        "alt_bad_quality": "图片 Alt 质量差", 
        "alt_bad_quality_desc": "Alt 文本使用了无意义词汇（如 image1.jpg, photo）或过短。", 
        "alt_bad_quality_impact": "搜索引擎无法理解图片内容，错失图片搜索流量，且对视障用户极不友好。", 
        
        "anchor_bad_quality": "锚文本质量差", 
        "anchor_bad_quality_desc": "使用了“点击这里”、“更多”等通用词汇作为链接文本。", 
        "anchor_bad_quality_impact": "无法向搜索引擎传递目标页面的关键词相关性，降低了目标页面的排名潜力。", 
        
        "cls_risk": "CLS 布局偏移风险 (静态检测)", 
        "cls_risk_desc": "检测到 `<img>` 标签缺失 `width` 或 `height` 属性。", 
        "cls_risk_impact": "图片加载时会撑开页面，导致布局发生意外抖动，直接恶化 CLS 指标。", 
        
        "missing_title": "缺失页面标题 (Title)", 
        "missing_title_desc": "页面代码中未找到 `<title>` 标签。", 
        "missing_title_impact": "Title 是最重要的 SEO 标签。缺失将导致搜索引擎无法判断页面主题，关键词排名极差。", 
        "missing_title_sugg": "为每个页面添加独特、包含核心关键词的标题。",
        
        "short_title": "标题过短", 
        "short_title_desc": "标题长度不足，难以完整表达页面意图。", 
        "short_title_impact": "浪费了宝贵的标题空间，错失了覆盖长尾关键词排名的机会。", 
        "short_title_sugg": "丰富标题内容，加入品牌词或修饰词，建议长度在 30-60 字符。",
        
        "long_title": "标题过长", 
        "long_title_desc": "标题超过 60 个字符。", 
        "long_title_impact": "在搜索结果中会被截断显示 (...)，不仅影响阅读，还可能降低用户点击欲望。", 
        "long_title_sugg": "精简标题长度，将核心信息前置，控制在 60 字符以内。",
        
        "missing_desc": "缺失元描述", 
        "missing_desc_desc": "页面未包含 `<meta name='description'>` 标签。", 
        "missing_desc_impact": "虽然不直接影响排名，但 Google 会随机抓取正文作为摘要，通常不可控且点击率低。", 
        "missing_desc_sugg": "添加吸引人的元描述，概括页面内容并包含号召性用语。",
        
        "short_desc": "元描述过短", 
        "short_desc_desc": "描述内容过少，无法形成完整的句子。", 
        "short_desc_impact": "无法充分展示页面卖点，在搜索结果中缺乏竞争力。", 
        "short_desc_sugg": "扩充描述至 120-155 字符，提供更多有价值的信息。",
        
        "missing_h1": "缺失 H1 标签", 
        "missing_h1_desc": "页面缺乏 `<h1>` 主标题。", 
        "missing_h1_impact": "搜索引擎难以理解内容的层级结构和核心主题，降低了关键词的相关性权重。", 
        "missing_h1_sugg": "确保每个页面有且仅有一个 H1 标签，概括当前页面的主题。",
        
        "missing_viewport": "缺失移动端视口配置", 
        "missing_viewport_desc": "未配置 `<meta name='viewport'>` 标签。", 
        "missing_viewport_impact": "在移动设备上显示异常（字体极小）。Google 移动优先索引会严重惩罚此类页面。", 
        "missing_viewport_sugg": "在 `<head>` 中添加标准的 viewport meta 标签。",
        
        "missing_canonical": "缺失 Canonical 标签", 
        "missing_canonical_desc": "未指定规范链接。", 
        "missing_canonical_impact": "无法应对 URL 参数（如 ?id=1）导致的重复内容问题，容易造成权重稀释。", 
        "missing_canonical_sugg": "在所有页面添加自引用（Self-referencing）或指向原件的 Canonical 标签。",
        
        "missing_jsonld": "缺失结构化数据", 
        "missing_jsonld_desc": "未检测到 Schema.org 标记。", 
        ...
        "missing_hreflang_desc": "未发现语言区域标记（HTML或Sitemap中均无）。", 
        "missing_hreflang_impact": "多语言站点无法正确定位目标受众，导致流量不精准。", 
        "missing_hreflang_sugg": "在 HTML 头部或 Sitemap 中配置 hreflang 标签。",
        
        "soft_404": "疑似软 404 (Soft 404)", 
        "soft_404_desc": "页面返回 200 状态码但内容显示“未找到”。", 
        "soft_404_impact": "严重浪费爬虫预算，导致无效页面挤占有效页面的索引名额。", 
        "soft_404_sugg": "配置服务器对不存在的页面返回 404 HTTP 状态码。",
        
        "missing_alt": "图片缺失 Alt 属性", 
        "missing_alt_desc": "图片标签缺少 `alt` 属性。", 
        "missing_alt_impact": "搜索引擎无法理解图片内容，错失图片搜索流量。", 
        "missing_alt_sugg": "为所有有意义的图片添加描述性的 alt 属性。",
        
        "js_links": "发现 JS 伪链接", 
        "js_links_desc": "使用了 `href='javascript:...'` 形式的链接。", 
        "js_links_impact": "爬虫无法跟踪此类链接，导致内部链接断裂，深层页面变成“孤岛”。", 
        "js_links_sugg": "使用标准的 `<a href='...'>` 标签，仅在 onclick 事件中处理 JS 逻辑。",
        
        "url_underscore": "URL 包含下划线", 
        "url_underscore_desc": "URL 路径中使用下划线 (`_`) 分隔单词。", 
        "url_underscore_impact": "Google 建议使用连字符。下划线可能导致关键词无法被正确切分（被视为一个长单词）。", 
        "url_underscore_sugg": "在 URL 结构中使用连字符 (`-`) 代替下划线。",
        
        "url_uppercase": "URL 包含大写字母", 
        "url_uppercase_desc": "URL 路径中混用了大写字母。", 
        "url_uppercase_impact": "服务器通常区分大小写，极易造成一页多址（Duplicate Content）和 404 错误。", 
        "url_uppercase_sugg": "强制所有 URL 使用小写字母。",
    },
    "en": {
        "sidebar_title": "🔍 AuditAI Pro",
        "sidebar_caption": "Deep Audit Edition v4.8",
        ...
        "input_info": "Note: v4.8 features professionally enriched descriptions and actionable advice.",
        ...
        # Issues
        "lcp_issue": "LCP Fails", 
        "lcp_desc": "LCP is {:.2f}s (Target <2.5s). Main content takes too long to appear.", 
        "lcp_impact": "LCP is a core ranking factor. Slow loading significantly increases bounce rates and hurts rankings.", 
        "lcp_sugg": "Optimize images (WebP), use a CDN, defer non-critical JS, and preload LCP elements.",
        
        "cls_issue": "CLS Fails", 
        "cls_desc": "CLS score is {:.3f} (Target <0.1). Elements shift unexpectedly during load.", 
        "cls_impact": "Ranking Factor. Poor visual stability frustrates users and leads to accidental clicks.", 
        "cls_sugg": "Set explicit width/height for images/videos and avoid inserting dynamic content above the fold.",
        
        "inp_issue": "INP Fails", 
        "inp_desc": "INP is {}ms (Target <200ms). Page is unresponsive to user interactions.", 
        "inp_impact": "New Core Vital. High latency makes the site feel 'broken', reducing conversion rates.", 
        "inp_sugg": "Break up long tasks, optimize event handlers, and minimize main-thread work.",
        
        "no_robots": "Missing Robots.txt", 
        "no_robots_desc": "No robots.txt found, or server returned an error.", 
        "no_robots_impact": "Crawlers may index useless pages, wasting crawl budget and server resources.", 
        "no_robots_sugg": "Create a valid robots.txt in the root directory.",
        
        "robots_bad_rule": "Robots.txt Blocking", 
        "robots_desc": "Detected a 'Disallow: /' rule blocking all agents.", 
        "robots_impact": "The entire site is de-indexed, resulting in zero organic traffic.", 
        
        "robots_no_sitemap": "Sitemap Missing in Robots", 
        "robots_no_sitemap_desc": "Sitemap location not specified in robots.txt.", 
        "robots_no_sitemap_impact": "Slows down the discovery of new or updated content by search engines.", 
        
        "no_sitemap": "Sitemap Failed", 
        "no_sitemap_desc": "Access denied (403/404) to Sitemap.", 
        "no_sitemap_impact": "Deep pages may not be discovered, leading to poor indexing coverage.", 
        
        "sitemap_invalid": "Invalid Sitemap", 
        "sitemap_invalid_desc": "XML parsing error.", 
        "sitemap_invalid_impact": "Search engines cannot read links, rendering the Sitemap useless.", 
        
        "no_favicon": "Missing Favicon", 
        "no_favicon_desc": "No icon found on homepage.", 
        "no_favicon_impact": "Reduces brand visibility and lowers Click-Through Rate (CTR) in search results.", 
        "no_favicon_sugg": "Add a favicon link tag to the <head>.",
        
        "duplicate": "Duplicate Content", 
        "duplicate_desc": "Exact duplicate found without proper canonical tag.", 
        "duplicate_impact": "Causes keyword cannibalization and dilutes link equity across duplicates.", 
        "duplicate_sugg": "Use rel='canonical' pointing to the original version.",
        
        "3xx_title": "Redirect Chain", 
        "3xx_desc": "Internal link triggers a redirect.", 
        "3xx_impact": "Wastes crawl budget, adds latency, and dilutes link equity.", 
        "3xx_sugg": "Update links to point directly to the final destination.",
        
        "4xx_title": "Broken Link", 
        "4xx_desc": "Internal link returns a 4xx error.", 
        "4xx_impact": "Bad user experience and loss of link equity flow.", 
        "4xx_sugg": "Fix or remove the broken link.",
        
        "5xx_title": "Server Error", 
        "5xx_desc": "Server returned a 5xx error.", 
        "5xx_impact": "Signals instability, causing Googlebot to reduce crawl rate.", 
        "5xx_sugg": "Check server logs and stability.",
        
        "hreflang_invalid": "Invalid Hreflang", 
        "hreflang_invalid_desc": "Language code format is invalid.", 
        "hreflang_invalid_impact": "Targeting fails, potentially showing wrong language versions in SERPs.", 
        
        "hreflang_no_default": "No x-default", 
        "hreflang_no_default_desc": "Missing 'x-default' fallback.", 
        "hreflang_no_default_impact": "Users from unmatched regions may be served an incorrect language.", 
        
        "alt_bad_quality": "Bad Alt Text", 
        "alt_bad_quality_desc": "Alt text uses filenames or generic words.", 
        "alt_bad_quality_impact": "Search engines cannot understand the image, hurting Image SEO.", 
        
        "anchor_bad_quality": "Bad Anchor", 
        "anchor_bad_quality_desc": "Generic anchor text found.", 
        "anchor_bad_quality_impact": "Fails to pass keyword relevance to the target page.", 
        
        "cls_risk": "CLS Risk (Static)", 
        "cls_risk_desc": "Images missing width/height attributes.", 
        "cls_risk_impact": "Causes layout shifts during load, negatively impacting Core Web Vitals.", 
        
        "missing_title": "Missing Title", 
        "missing_title_desc": "No <title> tag found.", 
        "missing_title_impact": "Severe ranking loss as search engines cannot determine page topic.", 
        "missing_title_sugg": "Add a unique, descriptive title.",
        
        "short_title": "Title Short", 
        "short_title_desc": "Title is too short.", 
        "short_title_impact": "Missed opportunity to target keywords.", 
        "short_title_sugg": "Expand title to include keywords.",
        
        "long_title": "Title Long", 
        "long_title_desc": "Title exceeds 60 characters.", 
        "long_title_impact": "Will be truncated in SERPs, reducing CTR.", 
        "long_title_sugg": "Shorten to under 60 characters.",
        
        "missing_desc": "Missing Description", 
        "missing_desc_desc": "No meta description found.", 
        "missing_desc_impact": "Lowers CTR as snippets will be auto-generated and often irrelevant.", 
        "missing_desc_sugg": "Add a compelling meta description.",
        
        "short_desc": "Description Short", 
        "short_desc_desc": "Description is too thin.", 
        "short_desc_impact": "Fails to attract clicks in SERPs.", 
        "short_desc_sugg": "Expand to 120-160 characters.",
        
        "missing_h1": "Missing H1", 
        "missing_h1_desc": "No <h1> tag found.", 
        "missing_h1_impact": "Search engines struggle to understand content hierarchy.", 
        "missing_h1_sugg": "Add a unique H1 tag.",
        
        "missing_viewport": "No Viewport", 
        "missing_viewport_desc": "Mobile viewport tag missing.", 
        "missing_viewport_impact": "Page is not mobile-friendly, leading to ranking drops.", 
        "missing_viewport_sugg": "Add standard viewport meta tag.",
        
        "missing_canonical": "No Canonical", 
        "missing_canonical_desc": "Missing canonical tag.", 
        "missing_canonical_impact": "High risk of duplicate content issues.", 
        "missing_canonical_sugg": "Add self-referencing canonical tag.",
        
        "missing_jsonld": "No Schema", 
        "missing_jsonld_desc": "No JSON-LD markup found.", 
        "missing_jsonld_impact": "Missed opportunity for Rich Snippets (Stars, Price, etc.).", 
        "missing_jsonld_sugg": "Add Schema based on page type.",
        
        "missing_hreflang": "No Hreflang", 
        "missing_hreflang_desc": "No language targeting tags.", 
        "missing_hreflang_impact": "Poor international targeting.", 
        "missing_hreflang_sugg": "Add hreflang tags.",
        
        "soft_404": "Soft 404", 
        "soft_404_desc": "Page returns 200 but content indicates error.", 
        "soft_404_impact": "Wastes crawl budget on invalid pages.", 
        "soft_404_sugg": "Configure server to return 404.",
        
        "missing_alt": "Missing Alt", 
        "missing_alt_desc": "Images lack alt attributes.", 
        "missing_alt_impact": "Bad for accessibility and Image SEO.", 
        "missing_alt_sugg": "Add descriptive alt text.",
        
        "js_links": "JS Links", 
        "js_links_desc": "Uncrawlable JavaScript links found.", 
        "js_links_impact": "Breaks internal link structure and equity flow.", 
        "js_links_sugg": "Use standard <a href> links.",
        
        "url_underscore": "URL Underscores", 
        "url_underscore_desc": "URL uses underscores.", 
        "url_underscore_impact": "Google prefers hyphens for word separation.", 
        "url_underscore_sugg": "Replace with hyphens.",
        
        "url_uppercase": "URL Uppercase", 
        "url_uppercase_desc": "URL contains uppercase letters.", 
        "url_uppercase_impact": "Risk of duplicate content due to case sensitivity.", 
    ...
    try:
        ...
        if r.status_code != 200:
            ...
            if "disallow: /" in content and "allow:" not in content:
                 issues.append({"id": "robots_bad_rule", "category": "access", "severity": "Critical", "title": t["robots_bad_rule"], "desc": t["robots_desc"], "impact": t["robots_impact"], "suggestion": "Remove global disallow rule.", "url": robots_url, "examples": [robots_url]})
            if "sitemap:" not in content:
                 issues.append({"id": "robots_no_sitemap", "category": "access", "severity": "Low", "title": t["robots_no_sitemap"], "desc": t["robots_no_sitemap_desc"], "impact": t["robots_no_sitemap_impact"], "suggestion": "Add 'Sitemap: [URL]' directive.", "url": robots_url, "examples": [robots_url]})
        r.close()
    except: 
        issues.append({"id": "no_robots", "category": "access", "severity": "Medium", "title": t["no_robots"], "desc": "Connection failed.", "impact": t["no_robots_impact"], "suggestion": "Check server config.", "url": robots_url, "examples": [robots_url]})
            ...
            if r.status_code == 200:
                try:
                    ...
                    if not sitemap_url.endswith('.gz'): 
                        issues.append({"id": "sitemap_invalid", "category": "access", "severity": "Medium", "title": t["sitemap_invalid"], "desc": t["sitemap_invalid_desc"], "impact": t["sitemap_invalid_impact"], "suggestion": "Check XML syntax.", "url": sitemap_url, "examples": [sitemap_url]})
            else:
                if manual_sitemaps:
                    issues.append({"id": "no_sitemap", "category": "access", "severity": "Low", "title": t["no_sitemap"], "desc": f"{t['no_sitemap_desc']} (Status: {r.status_code})", "impact": t["no_sitemap_impact"], "suggestion": "Check URL.", "url": sitemap_url, "examples": [sitemap_url]})
        except:
            if manual_sitemaps: issues.append({"id": "no_sitemap", "category": "access", "severity": "Low", "title": t["no_sitemap"], "desc": "Connection failed.", "impact": t["no_sitemap_impact"], "suggestion": "Check URL.", "url": sitemap_url, "examples": [sitemap_url]})

    if not any_sitemap_valid and not manual_sitemaps:
         issues.append({"id": "no_sitemap", "category": "access", "severity": "Low", "title": t["no_sitemap"], "desc": t["no_sitemap_desc"], "impact": t["no_sitemap_impact"], "suggestion": "Ensure sitemap.xml exists.", "url": sitemap_urls_to_check[0], "examples": [sitemap_urls_to_check[0]]})
...
def analyze_page(url, html_content, status_code, lang="zh", sitemap_has_hreflang=False):
    ...
    if hreflangs:
        ...
        if invalid_codes:
            issues.append({"id": "hreflang_invalid", "category": "indexability", "severity": "High", "title": t["hreflang_invalid"], "desc": f"{t['hreflang_invalid_desc']} ({', '.join(invalid_codes[:3])})", "impact": t["hreflang_invalid_impact"], "suggestion": "Use ISO 639-1 codes.", "url": url})
        if not has_x_default:
            issues.append({"id": "hreflang_no_default", "category": "indexability", "severity": "Low", "title": t["hreflang_no_default"], "desc": t["hreflang_no_default_desc"], "impact": t["hreflang_no_default_impact"], "suggestion": "Add hreflang='x-default'.", "url": url})
    ...
    if bad_alt_count > 0:
        issues.append({"id": "alt_bad_quality", "category": "image_ux", "severity": "Low", "title": t["alt_bad_quality"], "desc": t["alt_bad_quality_desc"], "impact": t["alt_bad_quality_impact"], "suggestion": "Avoid generic keywords.", "url": url, "evidence": f"{bad_alt_count} poor alts"})
    
    # 将 Static CLS Risk 归类为 cwv_performance
    if cls_risk_count > 0:
        issues.append({"id": "cls_risk", "category": "cwv_performance", "severity": "Medium", "title": t["cls_risk"], "desc": t["cls_risk_desc"], "impact": t["cls_risk_impact"], "suggestion": "Specify width/height.", "url": url, "evidence": f"{cls_risk_count} images"})
    ...
    if found_bad:
        issues.append({"id": "anchor_bad_quality", "category": "image_ux", "severity": "Low", "title": t["anchor_bad_quality"], "desc": f"{t['anchor_bad_quality_desc']} ({len(found_bad)})", "impact": t["anchor_bad_quality_impact"], "suggestion": "Use descriptive keywords.", "url": url})
