...
TRANSLATIONS = {
    "zh": {
        ...
        "input_header": "开始深度审计",
        "input_info": "说明: v9.7 优化了 Canonical 逻辑，非自规范化页面将跳过内容审计。",
        ...
        "ppt_cover_sub": "Generated by AuditAI Pro v9.7",
        ...
        "no_sitemap": "Sitemap 访问失败", "no_sitemap_desc": "无法访问 Sitemap 文件，服务器返回 4xx 或 5xx 错误。", "no_sitemap_impact": "Search engines will struggle to find deep links or orphan pages, leading to poor indexing coverage.", "no_sitemap_sugg": "Verify the Sitemap URL is correct and that server permissions allow external access.",
        "sitemap_invalid": "Sitemap 格式错误", "sitemap_invalid_desc": "XML 解析失败，文件格式不符合 standard protocol.", "sitemap_invalid_impact": "Search engines cannot read the links inside, rendering the Sitemap completely useless.", "sitemap_invalid_sugg": "Validate your XML syntax to ensure there are no unclosed tags or invalid characters.",
        ...
        "short_desc": "元描述过短", "short_desc_desc": "内容过少，吸引力不足。", "short_desc_impact": "无法充分展示页面卖点，在搜索结果中缺乏竞争力。", "short_desc_sugg": "扩充描述至 120-155 字符，提供更多有价值的信息。",
...
def analyze_page(url, content, status, sitemap_has_hreflang):
    ...
    if status == 200:
        ...
        if not can_url:
            ...
            # If missing, it's effectively self-canonical (the only one), so we audit it.
            is_self_canonical = True

        # Hreflang (Independent of canonical usually, but good to check)
        ...
        if hreflangs:
            ...
            if not has_x_default:
             ...
             # Only report missing hreflang if it's a self-canonical page
    ...
    while queue and count < max_pages:
        ...
        try:
            ...
            if response.history:
                ...
                # Smart Path Extraction for visualization (v9.5 Fix: No Truncation)
...
def draw_cwv_gauge(slide, metric, value, thresholds):
    ...
    total_w = 4
    x_start = 7.2
    y_pos = 4.8 # Moved up from 5.0 to 4.8
    h = 0.4
    ...
    def add_label(x_offset, text):
        tb = slide.shapes.add_textbox(Inches(x_start + x_offset - 0.2), Inches(y_pos + 0.45), Inches(0.5), Inches(0.3))
...
def create_styled_pptx(slides_data, lang):
        ...
        if "favicon" in issue_id:
             ...
             if len(parts) > 1:
                 ...
                 for i, part in enumerate(display_parts):
                     ...
                     b.text_frame.paragraphs[0].font.color.rgb = RGBColor(0,0,0) # Fix: Black font
    ...
    for s in slides_data:
        ...
        if is_code:
            ...
            # Pass full issue_id to help identify sub-type
            # Also pass evidence which holds invalid code if applicable
            type_str = s['id']
            if "invalid" in type_str and s.get('args'):
                type_str = f"invalid: {s['args'][0]}" # Pass the code
