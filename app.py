...
# --- 2. 基础工具函数 (核心修复：移至最顶层，杜绝 NameError) ---
...
ISSUE_PRIORITY_LIST = [
    "no_robots", "robots_bad_rule", "robots_quality_issue", "robots_no_sitemap", "no_sitemap", "sitemap_invalid",
...
TRANSLATIONS = {
    "zh": {
        ...
        "input_header": "开始深度审计",
        "input_info": "说明: v9.1 修复了 3xx 链接显示、Hreflang 真实示例及中文翻译遗漏。",
        ...
        "ppt_cover_sub": "Generated by AuditAI Pro v9.1",
        ...
        "long_title": "标题过长", "long_title_desc": "标题超过 60 个字符。", "long_title_impact": "标题将在搜索结果中被截断，降低可读性和点击率。", "long_title_sugg": "精简标题长度，将核心信息前置，控制在 60 字符以内。",
    ...
    "en": {
        "sidebar_title": "🔍 AuditAI Pro",
        "sidebar_caption": "Deep Audit Edition v8.9",
        ...
        "input_info": "Note: v8.9 features improved duplicate grouping and Robots.txt analysis.",
        ...
        "ppt_cover_sub": "Generated by AuditAI Pro v8.6",
        ...
        "hreflang_invalid_desc": "The language code format does not comply with ISO 639-1 standards (found: {}).", 
...
def analyze_page(url, content, status, sitemap_has_hreflang):
    ...
    if status == 200:
        ...
        if soup.find('a', href=lambda x: x and x.lower().startswith('javascript:')):
            issues.append({"id": "js_links", "category": "access", "severity": "High", "url": url}) # Moved to Access
        ...
        # Anchor Quality (Moved to Access)
    ...
    while queue and count < max_pages:
        ...
        try:
            ...
            if response.history:
                ...
                # Smart Path Extraction for visualization
                # Keep full filename, but maybe truncate long folders
                chain_str = " -> ".join([urlparse(u).path for u in chain_list]) 
...
def draw_cwv_gauge(slide, metric, value, thresholds):
    ...
    # Threshold Labels (0, 2.5, 4.0) - Positioned below bar
    def add_label(x_offset, text):
        tb = slide.shapes.add_textbox(Inches(x_start + x_offset - 0.2), Inches(y_pos + 0.5), Inches(0.5), Inches(0.3))
    ...
    tri = slide.shapes.add_shape(MSO_SHAPE.ISOSCELES_TRIANGLE, Inches(marker_x - 0.1), Inches(y_pos - 0.25), Inches(0.2), Inches(0.2))
    tri.rotation = 180
    tri.fill.solid()
    tri.fill.fore_color.rgb = RGBColor(50, 50, 50)
    tri.line.fill.background()
    
    # Value Label above marker (Prevent overlap)
    tb = slide.shapes.add_textbox(Inches(marker_x - 0.5), Inches(y_pos - 0.6), Inches(1), Inches(0.3))
    p = tb.text_frame.add_paragraph()
    p.text = f"{value}"
    p.font.size = Pt(11)
    p.font.bold = True
...
def create_styled_pptx(slides_data, lang):
    ...
    def draw_serp_preview(slide, issue_id, issue_title, evidence, url):
        ...
        if "favicon" in issue_id:
             ...
             # Speech Bubble logic based on missing vs bad quality
             bubble_text = '<img src="..." />' # Default for missing
             color = RGBColor(200, 0, 0)
             if "quality" in issue_id:
                 bubble_text = '<img src="..." alt="image" />' # Highlight bad word
             ...
             if len(parts) > 1:
                 ...
                 for i, part in enumerate(display_parts):
                     ...
                     b.text_frame.text = part[-15:] # Use relative path tail
             ...
             if "short_desc" in issue_id or "missing_desc" in issue_id:
                 ...
                 p_serp = tf.add_paragraph()
                 p_serp.space_before = Pt(5)
                 p_serp.text = evidence[:55] + " ..." # Simulate truncation
                 set_font(p_serp.font, 18, False, RGBColor(26, 13, 171))
    ...
    def draw_hreflang_preview(slide, url, bad_code):
        ...
        # Determine error type from args
        # analyze_page passes args as string "code1, code2"
        # We need to pass this arg to draw function.
        # But 'bad_code' here is issue_id string. We need args.
        # Fixed logic below in main loop to pass args.
        
        p = tf.add_paragraph()
        if "default" in bad_code:
             p.text = "<!-- Missing x-default -->"
             set_font(p.font, 10, False, RGBColor(200, 0, 0))
             p = tf.add_paragraph()
             p.text = f'<link rel="alternate" hreflang="x-default" href="{url}" />'
             set_font(p.font, 11, True, RGBColor(0, 128, 0))
        else:
             # Invalid code case
             p.text = f'<!-- Invalid code: {bad_code} -->'
             set_font(p.font, 10, False, RGBColor(200, 0, 0))
             p = tf.add_paragraph()
             p.text = f'<link rel="alternate" hreflang="{bad_code}" ... />'
             set_font(p.font, 11, False, RGBColor(128, 128, 128))
             p = tf.add_paragraph()
             p.text = '<!-- Should be ISO 639-1 (e.g., en-US) -->'
             set_font(p.font, 10, True, RGBColor(0, 128, 0))
    ...
    for s in slides_data:
        ...
        for idx, url in enumerate(s['examples'][:4]): 
            ...
            if s['id'] == 'duplicate' and "Duplicate Group:" in url:
                 ...
                 # "Duplicate Group:\n- url1\n- url2"
                 lines = url.split('\n')
                 p.text = lines[0] # Header
                 set_font(p.font, 11, True, RGBColor(0, 102, 204))
                 for sub_line in lines[1:]:
                     sub_p = tf.add_paragraph()
                     sub_p.text = sub_line
                     set_font(sub_p.font, 10, False, RGBColor(80, 80, 80))
            else:
                 p.text = f"• {url}"
        ...
        with st.container(border=True):
            ...
            with c2:
                ...
                if is_code:
                    ...
                    # Show the actual invalid code if available
                    err = ev if ev else "x-default"
                    if "default" in s['id']:
                        st.code(f'<link rel="alternate" hreflang="x-default" href="{ex_url}" />', language="html")
                    else:
                        st.code(f'<!-- Invalid -->\n<link rel="alternate" hreflang="{err}" ... />', language="html")
                elif is_rich:
