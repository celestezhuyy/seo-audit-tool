...
TRANSLATIONS = {
    "zh": {
        ...
        "input_header": "开始深度审计",
        "input_info": "说明: v7.6 增强了对 NXP 等大型企业站点的爬取能力，支持动态域名校准。",
        ...
        "ppt_cover_sub": "Generated by AuditAI Pro v7.6",
        ...
        "http_3xx": "内部链接重定向 (3xx)", "http_3xx_desc": "内部链接发生跳转 (Chain: {})。", "http_3xx_impact": "浪费爬虫预算，增加页面加载延迟，且每次跳转都会损耗少量链接传递的权重 (Link Equity)。", "http_3xx_sugg": "批量更新内部链接，使其直接指向最终的目标 URL，避免中间跳转。",
    ...
    "en": {
        "sidebar_title": "🔍 AuditAI Pro",
        "sidebar_caption": "Deep Audit Edition v7.6",
        ...
        "input_info": "Note: v7.6 features smart domain locking for better crawling.",
        ...
        "ppt_cover_sub": "Generated by AuditAI Pro v7.5",
...
def check_site_level_assets(start_url, lang="zh", check_robots=True, crawl_sitemap_flag=True, manual_sitemaps=None):
    ...
    # 获取真正的 base_domain（从 start_url 中提取，忽略 www）
    # 使用函数内的局部变量，不要依赖全局变量
...
def crawl_website(start_url, max_pages, lang, manual_robots, manual_sitemaps, psi_key, list_url=None, detail_url=None, check_robots=True, crawl_sitemap=True):
    ...
    # Priority Queue (Manual URLs first)
    queue = [start_url]
    ...
    # --- SMART DOMAIN LOCKING (Updated Logic) ---
    # 1. First, determine the 'base_domain' from the start_url
    start_netloc = urlparse(start_url).netloc.replace('www.', '')
    
    def is_internal_link(base, link):
        try:
            # 2. Extract netloc of link
            l_netloc = urlparse(link).netloc.replace('www.', '')
            # 3. Handle relative links (empty netloc) -> Treat as internal
            if not l_netloc: return True 
            # 4. Check if link domain ENDS WITH base domain (handles subdomains)
            # e.g., 'blog.nxp.com' ends with 'nxp.com' -> True
            return l_netloc == base or l_netloc.endswith('.' + base)
        except: return False
    ...
    while queue and count < max_pages:
        ...
        try:
            ...
            current_url = response.url # Actual URL after redirects
            
            # **DYNAMIC DOMAIN CALIBRATION**
            # If this is the FIRST page (start_url), and it redirected, update the base_domain!
            if count == 1 and url == start_url:
                 # Update base_domain to the redirected location (e.g., nxp.com -> nxp.com.cn)
            ...
            if 'text/html' in content_type:
                ...
                if final_status == 200:
                    ...
                    if current_hash in seen_hashes:
                        ...
                        if not (current_canonical and current_canonical != current_url):
                            all_issues.append({
                ...
                for a in soup.find_all('a', href=True):
                    ...
                    # Using updated start_netloc which accounts for initial redirect
                    if is_internal_link(start_netloc, link) and link not in seen_urls:
                        # Exclude non-HTML assets
...
def create_styled_pptx(slides_data, lang):
    ...
    def draw_serp_preview(slide, issue_title, evidence, url):
        ...
        if "favicon" in issue_title.lower():
             p.text = txt["visual_sim_title"]
             # Draw No Favicon Globe
             circle = slide.shapes.add_shape(MSO_SHAPE.OVAL, Inches(7.2), Inches(4.2), Inches(0.5), Inches(0.5))
             circle.fill.fore_color.rgb = RGBColor(200, 200, 200) # Grey Globe
             
             tb = slide.shapes.add_textbox(Inches(7.8), Inches(4.2), Inches(4), Inches(1))
             p2 = tb.text_frame.add_paragraph()
             p2.text = "No Icon"
             set_font(p2.font, 14, True, RGBColor(100, 100, 100))

        elif "alt" in issue_title.lower():
             p.text = txt["visual_sim_title"]
             # Draw Broken Image
             img_box = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(7.2), Inches(4.2), Inches(1.5), Inches(1.5))
             img_box.line.color.rgb = RGBColor(200, 200, 200)
             img_box.fill.background() # No fill
             tf = img_box.text_frame
             p_img = tf.add_paragraph()
             p_img.text = "[IMG] product.jpg"
             set_font(p_img.font, 10, False, RGBColor(150, 150, 150))
             
        elif "lcp" in issue_title.lower() or "inp" in issue_title.lower() or "cls" in issue_title.lower():
             p.text = "Core Web Vitals Visual:"
             # Draw Bar Chart Background
             bg_bar = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(7.2), Inches(4.5), Inches(5), Inches(0.4))
             bg_bar.fill.fore_color.rgb = RGBColor(230, 230, 230)
             # Draw Red Bar
             bar = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(7.2), Inches(4.5), Inches(2.5), Inches(0.4))
             bar.fill.fore_color.rgb = RGBColor(220, 53, 69) # Red bar for Poor
             
             label_box = slide.shapes.add_textbox(Inches(7.2), Inches(5.0), Inches(5), Inches(0.5))
             p_bar = label_box.text_frame.add_paragraph()
             p_bar.text = "⚠️ Poor Performance Zone"
             set_font(p_bar.font, 12, True, RGBColor(220, 53, 69))
             
        elif "3xx" in issue_title.lower():
             p.text = "Redirect Path:"
             # Draw Flow
             box1 = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(7.2), Inches(4.2), Inches(2), Inches(0.6))
             box1.text_frame.text = "Origin"
             
             arrow = slide.shapes.add_shape(MSO_SHAPE.RIGHT_ARROW, Inches(9.3), Inches(4.4), Inches(0.8), Inches(0.2))
             
             box2 = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(10.2), Inches(4.2), Inches(2), Inches(0.6))
             box2.text_frame.text = "Final"
             box2.fill.fore_color.rgb = RGBColor(40, 167, 69) # Green
    ...
    for s in slides_data:
        ...
        elif is_img or is_cwv or is_3xx:
            draw_serp_preview(slide, t_data['title'], ev, ex_url) # General visual
