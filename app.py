...
def estimate_pixel_width(text, font_size=18):
    """估算文本在 Google SERP 中的像素宽度 (Arial 18px 近似)"""
    if not text: return 0
    width = 0
    for char in text:
        # CJK 字符或全角字符通常占满一个字号宽度
        if ord(char) > 127: 
            width += font_size
        elif char.isupper():
            width += font_size * 0.7  # 大写字母稍宽
        else:
            width += font_size * 0.55 # 小写字母及数字
    return width
...
TRANSLATIONS = {
    "zh": {
        ...
        "sidebar_caption": "旗舰审计版 v9.9 (Final)",
        "nav_label": "功能导航",
        ...
        "input_header": "开始深度审计",
        "input_info": "说明: v9.9 引入了基于像素的标题长度计算，并优化了所有可视化图表。",
        ...
        "ppt_cover_sub": "Generated by AuditAI Pro v9.9",
        ...
        "short_title": "标题过短", "short_title_desc": "标题长度不足 (约 {} px)，难以完整表达页面意图。", "short_title_impact": "浪费了宝贵的标题空间，错失了覆盖长尾关键词排名的机会。", "short_title_sugg": "丰富标题内容，加入品牌词或修饰词，建议长度 30-60 字符。",
        "long_title": "标题过长", "long_title_desc": "标题超过建议显示宽度 (约 {} px)。", "long_title_impact": "标题将在搜索结果中被截断，降低可读性和点击率。", "long_title_sugg": "精简标题长度，将核心信息前置，控制在 600px 以内。",
        ...
        "short_desc": "元描述过短", "short_desc_desc": "描述内容过少 (约 {} px)，吸引力不足。", "short_desc_impact": "无法充分展示页面卖点，在搜索结果中缺乏竞争力。", "short_desc_sugg": "扩充描述至 120-155 字符，提供更多有价值的信息。",
    ...
    "en": {
        "sidebar_title": "🔍 AuditAI Pro",
        "sidebar_caption": "Deep Audit Edition v9.9",
        ...
        "input_info": "Note: v9.9 features pixel-based title checks & optimized visuals.",
        ...
        "short_title_sugg": "Expand the title to ~300-500px, including your brand name.",
        ...
        "short_desc_sugg": "Expand the description to 400-900px with a call to action.",
...
def create_styled_pptx(slides_data, lang):
    ...
    def draw_serp_preview(slide, issue_id, issue_title, evidence, url):
        ...
        if "favicon" in issue_id:
             ...
             # Add Cross lines to simulate globe
