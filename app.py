...
TRANSLATIONS = {
    "zh": {
        ...
        "input_header": "开始深度审计",
        "input_info": "说明: 升级版重复内容识别逻辑（智能区分参数页与主页），修复误判问题。",
        "input_label": "输入目标网址",
        ...
        "ppt_info": "说明：生成的 PPT 已优化为 16:9 宽屏，包含 SERP 预览和深度代码分析。",
        ...
        "ppt_cover_sub": "Generated by AuditAI Pro v3.5",
    ...
    "en": {
        "sidebar_title": "🔍 AuditAI Pro",
        "sidebar_caption": "Deep Audit Edition v3.5",
        ...
        "input_info": "Note: Improved Deduplication Logic (Smart Main/Param page detection).",
...
def analyze_page(url, html_content, status_code, lang="zh", sitemap_has_hreflang=False):
    ...
    txt = {
        "zh": {
            ...
            "duplicate": "发现未规范化的重复内容", "duplicate_desc": "内容高度重复且未指向同一 Canonical URL。导致权重分散。", "duplicate_sugg": "使用 Canonical 指向原始页面。",
        ...
        "en": {
            ...
            "duplicate": "Uncanonicalized Duplicate", "duplicate_desc": "Duplicate content not pointing to same Canonical URL.", "duplicate_sugg": "Point Canonical to original.",
...
def crawl_website(start_url, max_pages=100, lang="zh", manual_robots=None, manual_sitemaps=None, psi_key=None):
    ...
    t_dup_title = "Duplicate Content" if lang == "en" else "发现未规范化的重复内容"
    t_dup_desc = "Uncanonicalized duplicate." if lang == "en" else "内容重复且 Canonical 未指向同一页面。"
    ...
    # 2. CWV (Real User Data via API - Home Only)
    cwv_data = None
    if psi_key:
    ...
    while queue and pages_crawled < max_pages:
        url = queue.pop(0)
        # 移除 visited 检查，因为我们在入队时已经检查了 seen_urls
        ...
        try:
            ...
            current_url = response.url # 获取重定向后的最终URL
            
            # 如果是第一次爬取，锁定该域名
            if target_domain is None:
                target_domain = urlparse(current_url).netloc
            
            content_type = response.headers.get('Content-Type', '').lower()
            if 'text/html' in content_type:
                page_data, page_issues, _ = analyze_page(
                    current_url, response.content, response.status_code, 
                ...
                if current_hash in seen_hashes:
                    ...
                    # 1. 判断是否只是参数页
                    is_param_variant = current_clean == original_clean
                    
                    # 2. 判断是否 "更干净" (我们想保留更短、无参数的作为 original)
                    # 如果当前 URL 比原 URL 更短且无参数，则把当前 URL 视为新的 original
                    if len(current_url) < len(original_url) and '?' not in current_url and '?' in original_url:
                        ...
                        # 当前页面可能是副本
                        # 检查 Canonical 是否正确指向
                        # 如果 canonical 指向的不是自己，而是 original (或者 original 也是 param 页但 canonical 指向了真正的 base)
                        # 只要 canonical 存在且 != current_url，我们就认为处理得当
                        
                        is_properly_handled = False
                        if current_canonical and current_canonical != current_url:
                            is_properly_handled = True
                        
                        if not is_properly_handled:
                            # 确实是问题：内容重复，且没有通过 Canonical 指向别处
                            page_issues.append({
                                "severity": "High", "title": t_dup_title, 
                                "desc": f"{t_dup_desc} (Original: {original_url})", 
                                "suggestion": f"{t_dup_sugg} {original_url}", 
                                "url": current_url, "meta": f"Duplicate of: {original_url}"
                            })
                else:
                    seen_hashes[current_hash] = current_url
                ...
                # Link Extraction with domain locking
                soup = BeautifulSoup(response.content, 'html.parser')
                for a in soup.find_all('a', href=True):
                    link = urljoin(current_url, a['href'])
                    
                    # 过滤逻辑：
                    # 1. 域名必须匹配 target_domain
                    # 2. 不能是静态资源
                    # 3. 之前没见过
...
def create_styled_pptx(slides_data, lang="zh"):
    ...
    p.alignment = PP_ALIGN.CENTER
    set_font(p.font, 24, False, RGBColor(200, 200, 200)) # Fixed
