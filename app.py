...
TRANSLATIONS = {
    "zh": {
        ...
        "input_header": "开始深度审计",
        "input_info": "说明: v7.9 包含 CWV 进度条可视化、Hreflang 动态代码示例及爬取范围控制。",
        ...
        "allow_subdomains_label": "允许抓取子域名 (如 blog.site.com)", # New
        "allow_outside_folder_label": "允许抓取父级目录 (如从 /en/ 开始抓取 /fr/)", # New
        ...
        "ppt_cover_sub": "Generated by AuditAI Pro v7.9",
    ...
    "en": {
        "sidebar_title": "🔍 AuditAI Pro",
        "sidebar_caption": "Deep Audit Edition v7.9",
        ...
        "input_info": "Note: v7.9 features smart sub-domain & path crawling control.",
        ...
        "allow_subdomains_label": "Allow Subdomains (e.g. blog.site.com)", # New
        "allow_outside_folder_label": "Allow Outside Start Folder (e.g. /fr/ from /en/)", # New
    ...
    while queue and count < max_pages:
        ...
        try:
            ...
            if response.history:
                ...
                if len(chain) > 50: chain = chain[:20] + "..." + chain[-20:]
...
def draw_cwv_gauge(slide, metric, value, thresholds):
    ...
    # Marker Logic
    # Map value to position 0..1 (approx)
    # LCP: 0..6s mapped to 0..1
    # INP: 0..600ms
    # CLS: 0..0.5
    
    limit = poor * 1.5 # Cap the max value for visual
    if value > limit: value = limit
    pos_ratio = value / limit
    marker_x = x_start + (pos_ratio * total_w)
...
def create_styled_pptx(slides_data, lang):
    ...
    def draw_serp_preview(slide, issue_title, evidence, url):
        ...
        if "favicon" in issue_title.lower():
             ...
             p2.text = "No Icon (Generic Earth)"
             set_font(p2.font, 14, True, RGBColor(100, 100, 100))

        elif "alt" in issue_title.lower():
             p.text = "Bot View vs User View"
             set_font(p.font, 12, True, RGBColor(100, 100, 100))
             
             # Bot Code
             code_box = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(7.2), Inches(4.2), Inches(5), Inches(0.5))
             code_box.fill.solid()
             code_box.fill.fore_color.rgb = RGBColor(30, 30, 30)
             tfc = code_box.text_frame
             pc = tfc.add_paragraph()
             pc.text = '<img src="img.jpg" /> <!-- Missing Alt -->'
             set_font(pc.font, 12, False, RGBColor(0, 255, 0))

             # Broken Image
             img_box = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(7.2), Inches(5.0), Inches(1.5), Inches(1.5))
             img_box.line.color.rgb = RGBColor(200, 200, 200)
             img_box.fill.background() 
             tf = img_box.text_frame
             p_img = tf.add_paragraph()
             p_img.text = "❌ Image Broken"
             set_font(p_img.font, 10, False, RGBColor(150, 150, 150))
             
        elif "lcp" in issue_title.lower():
             p.text = txt["cwv_sim_title"] + " LCP"
             set_font(p.font, 12, True, RGBColor(100, 100, 100))
             # Parse value from title or evidence (simplified)
             val = float(re.findall(r"(\d+\.\d+)", evidence)[0]) if re.findall(r"(\d+\.\d+)", evidence) else 3.0
             draw_cwv_gauge(slide, "LCP", val, (2.5, 4.0))

        elif "inp" in issue_title.lower():
             ...
             # Draw Flow (Dynamic based on evidence if possible)
             # Evidence contains "A -> B -> C"
             parts = evidence.split(' -> ')
             if len(parts) > 1:
                 x = 7.2
                 for i, part in enumerate(parts[:3]): # Max 3 nodes
                     b = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(x), Inches(4.5), Inches(1.5), Inches(0.6))
                     b.text_frame.text = "..." + part[-15:] # Truncate
                     b.text_frame.paragraphs[0].font.size = Pt(10)
                     x += 1.8
                     if i < min(len(parts), 3) - 1:
                         ar = slide.shapes.add_shape(MSO_SHAPE.RIGHT_ARROW, Inches(x-0.3), Inches(4.7), Inches(0.3), Inches(0.2))
                         ar.fill.solid()
             ...
             # Short desc logic
             if "short_desc" in issue_title.lower() or "missing_desc" in issue_title.lower():
                 p_serp = tf.add_paragraph()
                 ...
                 if "missing" in issue_title.lower():
                    p_serp.text = "No description available in code..."
                 else:
                    p_serp.text = evidence # Show the short description
    ...
    for s in slides_data:
        ...
        # For 3xx, extract full chain if available in args
        ...
        if is_code:
            ...
            draw_serp_preview(slide, s['id'], ev, ex_url)
    ...
    if not st.session_state['audit_issues']: st.warning(ui["warn_no_data"])
        ...
        with st.container(border=True):
            ...
            with c2:
                ...
                ex_url = s['examples'][0] if s['examples'] else "example.com"
                if "Duplicate" in ex_url: ex_url = ex_url.split("Duplicate of:")[0].strip()
                if "3xx" in s['id'] and s.get('args'): ev = s['args'][0] # Use chain for visual
