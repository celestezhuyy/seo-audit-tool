...
ISSUE_PRIORITY_LIST = [
    ...
    "http_5xx", "http_4xx", "soft_404", "http_3xx", "js_links", "anchor_bad_quality",
    # 2. Indexability
    "duplicate", "missing_canonical", "hreflang_invalid", "hreflang_no_default", "missing_hreflang",
    # 3. Technical
    "missing_viewport", "missing_jsonld", "url_underscore", "url_uppercase",
    # 4. Content
    "missing_title", "short_title", "long_title", "missing_desc", "short_desc", "missing_h1",
    # 5. UX
    "no_favicon", "missing_alt", "alt_bad_quality", 
    # 6. CWV
...
TRANSLATIONS = {
    "zh": {
        ...
        "input_header": "开始深度审计",
        "input_info": "说明: v8.5 针对3xx/Alt/CWV/锚文本等问题进行了深度可视化优化。",
        ...
        "ppt_cover_sub": "Generated by AuditAI Pro v8.5",
        ...
        "missing_jsonld": "缺失结构化数据", "missing_jsonld_desc": "未检测到 Schema.org 标记。", "missing_jsonld_impact": "错失富媒体搜索结果（Rich Results），在 SERP 中不如竞争对手显眼。", "missing_jsonld_sugg": "建议根据页面类型（{}）添加对应的 JSON-LD Schema。",
    ...
    "en": {
        "sidebar_title": "🔍 AuditAI Pro",
        "sidebar_caption": "Deep Audit Edition v8.5",
        ...
        "input_info": "Note: v8.5 features enhanced visuals for Favicon, Hreflang & CWV.",
...
def analyze_page(url, content, status, sitemap_has_hreflang):
    ...
    return {
        "URL": url, "Status": status, "Title": soup.title.string.strip() if soup.title else "N/A", 
...
def create_styled_pptx(slides_data, lang):
    ...
    def draw_hreflang_preview(slide, url, missing_type):
        box = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(7), Inches(4), Inches(5.8), Inches(1.8))
        box.fill.solid()
        box.fill.fore_color.rgb = RGBColor(245, 245, 245)
        box.line.color.rgb = RGBColor(200, 200, 200)
    ...
    def draw_rich_snippet_preview(slide, url):
        box = slide.shapes.add_shape(MSO_SHAPE.ROUNDED_RECTANGLE, Inches(7), Inches(4), Inches(5.8), Inches(1.8)) # Moved down
        box.fill.solid() # Fix
        box.fill.fore_color.rgb = RGBColor(255, 255, 255)
        box.line.color.rgb = RGBColor(220, 220, 220)
        tf = box.text_frame
