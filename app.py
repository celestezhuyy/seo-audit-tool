...
TRANSLATIONS = {
    "zh": {
        ...
        "ppt_cover_sub": "Generated by AuditAI Pro v7.3",
    ...
    "en": {
        ...
        "input_info": "Note: v7.3 features enhanced 3xx visualization and layout fixes.",
...
def create_styled_pptx(slides_data, lang):
    ...
    def draw_serp_preview(slide, issue_title, evidence, url):
        ...
        if "favicon" in issue_title.lower():
             ...
             p.text = txt["visual_sim_title"]
             # Draw Broken Image
             img_box = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(7.2), Inches(4.2), Inches(1), Inches(1))
             ...
             # Draw Bar Chart
             bar = slide.shapes.add_shape(MSO_SHAPE.RECTANGLE, Inches(7.2), Inches(4.5), Inches(4), Inches(0.5))
             bar.fill.fore_color.rgb = RGBColor(220, 53, 69) # Red bar for Poor
             tf = bar.text_frame
             p_bar = tf.add_paragraph()
             p_bar.text = "Poor Performance Zone"
    ...
    def draw_code_preview(slide):
        ...
        tf = box.text_frame
        p = tf.add_paragraph()
        p.text = '<a href="javascript:void(0)">Click Here</a>'
