...
# 尝试导入 pptx，如果报错则提示用户更新 requirements.txt
try:
    from pptx import Presentation
    from pptx.util import Inches, Pt
except ImportError:
    st.error("缺失依赖库！请在 requirements.txt 中添加 'python-pptx' 并重启应用。")
    st.stop()
...
def analyze_page(url, html_content, status_code):
    """分析单个页面的SEO指标，返回数据、问题列表和内容指纹"""
    soup = BeautifulSoup(html_content, 'html.parser')
    issues = []
    
    # 提取纯文本用于分析和去重
    # 移除 script 和 style 标签，只保留正文
    for script in soup(["script", "style"]):
        script.extract()
    text_content = soup.get_text().strip()
    text_content_lower = text_content.lower()
    content_hash = get_content_hash(text_content) # 计算指纹
    ...
    if not title:
        issues.append({
         ...
         issues.append({
            "severity": "Low",
            "title": "标题过长",
            "desc": f"标题长达 {len(title)} 字符，可能会被截断。",
    ...
    if not schema:
         issues.append({
            ...
            "desc": "结构化数据有助于 Google 理解内容并生成富媒体搜索结果。",
            "suggestion": "添加适合页面的 JSON-LD 结构化数据。",
            "url": url
        })
    ...
    if '_' in path:
         issues.append({
            "severity": "Low",
            "title": "URL 包含下划线",
            "desc": "Google 建议在 URL 中使用连字符 (-) 而非下划线 (_) 。",
    ...
    if js_links:
        issues.append({
            ...
            "desc": f"发现 {len(js_links)} 个链接使用 href='javascript:'，爬虫无法跟踪。",
            "suggestion": "使用标准的 <a href='URL'> 标签。",
            "url": url,
    ...
    # 11. 软 404 检测 (增强版)
    # 检查状态码为 200 时，页面文本或标题是否包含错误信息
    is_soft_404 = False
    error_keywords = ["page not found", "404 error", "page cannot be found", "页面未找到", "404 错误"]
    
    # 检查 Title
    if title and any(k in title.lower() for k in error_keywords):
        is_soft_404 = True
    # 检查 H1
    elif h1_text != "No H1" and any(k in h1_text.lower() for k in error_keywords):
        is_soft_404 = True
    # 检查正文
    elif any(k in text_content_lower[:500] for k in error_keywords): # 仅检查前500字符，避免误判
        is_soft_404 = True
    ...
    if status_code == 200 and is_soft_404:
        issues.append({
            ...
            "desc": "页面返回 200 状态码，但 Title 或内容显示为错误页面。这会严重浪费爬取预算。",
            "suggestion": "配置服务器，对不存在的页面必须返回 404 HTTP 状态码。",
            "url": url
        })
        ...
        try:
            ...
            if 'text/html' in content_type:
                ...
                if current_hash in seen_hashes:
                    ...
                    page_issues.append({
                        "severity": "High",
                        "title": "发现重复内容 (Duplicate Content)",
                        "desc": f"此页面内容与 {original_url} 高度一致。",
                        "suggestion": "使用 Canonical 标签指向原始版本，或合并页面。",
                        "url": url,
...
def create_pptx_file(slides_data):
    """生成 PPT 二进制文件"""
    prs = Presentation()
    ...
    title.text = "SEO 技术审计报告"
    subtitle.text = "Generated by NextGen SEO Auditor"
    ...
    for issue in slides_data:
        ...
        title_shape.text = issue['title']
        
        # 内容
        body_shape = shapes.placeholders[1]
        tf = body_shape.text_frame
        tf.text = f"严重程度: {issue['severity']}"
        
        p = tf.add_paragraph()
        p.text = f"影响页面数: {issue['count']}"
        p.level = 1
...
with st.sidebar:
    ...
    st.caption("Live Crawler Edition v2.1")
...
if menu == "输入网址":
    st.header("开始新的审计")
    st.info("说明: 包含重复内容检测、软404增强检测及PPT导出功能。")
    ...
    if st.session_state['audit_data'] is None:
        ...
        for issue in raw_issues:
            ...
            if len(grouped_issues[title]['examples']) < 5: # PPT 中多展示几个例子
        ...
        # --- 下载按钮 ---
        st.write("### 📥 导出报告")
        if st.button("生成并下载 .pptx 文件"):
            with st.spinner("正在生成 PPT 文件..."):
                pptx_file = create_pptx_file(ppt_slides)
                st.download_button(
                    label="点击下载 SEO 审计报告 (PPTX)",
                    data=pptx_file,
                    file_name="seo_audit_report.pptx",
