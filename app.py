...
TRANSLATIONS = {
    "zh": {
        ...
        "input_header": "开始深度审计",
        "input_info": "说明: v6.0 修复了所有已知的变量名错误，并优化了报告逻辑顺序。",
        "input_label": "输入目标网址",
        ...
        "ppt_cover_sub": "Generated by AuditAI Pro v6.0",
    ...
    "en": {
        "sidebar_title": "🔍 AuditAI Pro",
        "sidebar_caption": "Deep Audit Edition v6.0",
        ...
        "input_info": "Note: v6.0 features strict logic, full English/Chinese separation, and detailed recommendations.",
...
if menu_key == "input":
    ...
    with st.expander(ui.get("psi_settings", "Google PSI")):
        key = st.text_input(ui.get("psi_api_key_label", "API Key"), type="password", help=ui.get("psi_api_help", ""))
    ...
    if st.button(ui["start_btn"], type="primary"):
        if not target_url or not is_valid_url(target_url): 
            ...
            with st.spinner(ui["spinner_crawl"].format(max_p)):
                data, issues, error_msg = crawl_website(target_url, max_p, lang, manual_robots, manual_sitemaps, key if 'key' in locals() else None)
                if not data:
