...
TRANSLATIONS = {
    "zh": {
        ...
        "input_header": "开始新的审计",
        "input_info": "说明: 新增 HTTP 3xx (重定向链) 与 5xx (服务器错误) 深度检测。",
        "input_label": "输入目标网址",
        ...
        "ppt_info": "说明：生成的 PPT 已优化为 16:9 宽屏，针对标题/描述问题包含 Google SERP 模拟预览。",
        ...
        "ppt_slide_ex_title": "受影响页面示例 & SERP 预览",
        "ppt_slide_sugg_title": "💡 修复建议:",
        "serp_sim_title": "Google 搜索结果模拟:",
    },
    "en": {
        "sidebar_title": "🔍 AuditAI Pro",
        "sidebar_caption": "Live Crawler Edition v2.9",
        ...
        "input_info": "Note: Added deep inspection for HTTP 3xx (Redirect Chains) & 5xx (Server Errors).",
        ...
        "ppt_info": "Note: PPT optimized for 16:9 with Google SERP simulation for title/meta issues.",
        ...
        "ppt_slide_ex_title": "Example URLs & SERP Preview",
...
def analyze_page(url, html_content, status_code, lang="zh", sitemap_has_hreflang=False):
    ...
    txt = {
        "zh": {
            ...
            "long_title_sugg": "精简标题长度 (建议<60字符)。",
            "missing_desc": "缺失元描述",
        ...
        "en": {
            ...
            "long_title_sugg": "Shorten the title (Recommend < 60 chars).",
    ...
    if not schema:
         ...
         elif "product" in path or "shop" in path: spec_rec = "Product"
         elif "blog" in path or "news" in path: spec_rec = "Article / BlogPosting"
         else: spec_rec = "BreadcrumbList"
    ...
    if not hreflang and not sitemap_has_hreflang:
...
def crawl_website(start_url, max_pages=100, lang="zh", manual_robots=None, manual_sitemaps=None):
    ...
    t_3xx_desc = "Internal link redirects to another URL." if lang == "en" else "内部链接发生跳转，浪费爬虫预算。"
    t_3xx_sugg = "Update link to point directly to destination." if lang == "en" else "更新链接直接指向最终目标。"
    
    t_4xx_title = "Broken Link (4xx)" if lang == "en" else "死链/客户端错误 (4xx)"
    t_4xx_desc = "Internal link returns 4xx error." if lang == "en" else "内部链接返回 4xx 错误 (如 404)。"
    t_4xx_sugg = "Fix or remove the broken link." if lang == "en" else "修复或移除该死链。"
    
    t_5xx_title = "Server Error (5xx)" if lang == "en" else "服务器错误 (5xx)"
    t_5xx_desc = "Internal link returns 5xx server error." if lang == "en" else "服务器返回 5xx 错误，表明服务器端存在故障。"
    t_5xx_sugg = "Check server logs and application stability." if lang == "en" else "检查服务器日志及应用稳定性。"
    ...
    while queue and pages_crawled < max_pages:
        ...
        try:
            headers = get_browser_headers()
            # 关键：allow_redirects=True 会自动跟随，但 response.history 会记录过程
            ...
            if response.history:
                ...
                all_issues.append({
                    "severity": "Medium",
                    "title": t_3xx_title,
                    "desc": f"{t_3xx_desc} (Redirects to: {response.url})",
                    "suggestion": t_3xx_sugg,
                    "url": url # 原始 URL
                })
            
            # --- 2. 检测 4xx/5xx ---
            if final_status >= 400:
                is_server_err = final_status >= 500
                all_issues.append({
                    "severity": "Critical" if is_server_err else "High",
                    "title": t_5xx_title if is_server_err else t_4xx_title,
                    "desc": f"{t_5xx_desc if is_server_err else t_4xx_desc} (Status: {final_status})",
                    "suggestion": t_5xx_sugg if is_server_err else t_4xx_sugg,
                    "url": url # 这里通常是最终 URL（如果是重定向后报错）或者原始 URL
                })
            
            # 即使报错，如果内容是 HTML，也尝试分析（比如自定义 404 页面可能也有 SEO 问题）
            content_type = response.headers.get('Content-Type', '').lower()
            if 'text/html' in content_type:
                page_data, page_issues, new_links = analyze_page(
                    url, response.content, final_status, 
                    lang=lang, sitemap_has_hreflang=sitemap_has_hreflang
                )
                
                # --- 重复内容检测 ---
                if final_status == 200: # 只对 200 页面做去重检查
                    ...
                    if current_hash in seen_hashes:
                        ...
                        if not is_properly_canonicalized:
                            page_issues.append({
                                ...
                                "suggestion": f"{t_dup_sugg} {original_url}", "url": url, "meta": f"Duplicate of: {original_url}"
                            })
                    else:
...
def create_styled_pptx(slides_data, lang="zh"):
    ...
    # Helper: Draw Google SERP Preview
    def draw_serp_preview(slide, issue_title, evidence, url):
        ...
        # 2. Title (Blue, simulates truncation)
        p = tf.add_paragraph()
        p.space_before = Pt(5)
        
        # 模拟标题逻辑
        display_title = evidence if evidence else "Untitled Page"
        if "Title Too Long" in issue_title or "标题过长" in issue_title:
            # 强制截断并加 ...
            if len(display_title) > 60:
                display_title = display_title[:55] + " ..."
        elif "Title Too Short" in issue_title or "标题过短" in issue_title:
            pass # Keep as is
        elif "Missing Title" in issue_title or "缺失页面标题" in issue_title:
            display_title = "Untitled Page"

        p.text = display_title
        set_font(p.font, 18, False, RGBColor(26, 13, 171)) # Google Blue
        
        # 3. Description
        p = tf.add_paragraph()
        p.space_before = Pt(3)
        # 模拟描述
        desc_text = "Please provide a meta description to help search engines understand your page content..."
        if "Meta Description" in issue_title or "元描述" in issue_title:
             if "Missing" in issue_title or "缺失" in issue_title:
                 desc_text = "NO DESCRIPTION FOUND. Google will try to extract text from the page body, which may not be optimal..."
             else:
                 desc_text = evidence if evidence else desc_text
        
        p.text = desc_text
        set_font(p.font, 14, False, RGBColor(77, 81, 86)) # Google Grey
    ...
    for issue in slides_data:
        ...
        # Check if we should draw SERP Preview (For Title/Meta issues)
        is_serp_issue = any(k in issue['title'] for k in ["Title", "标题", "Meta Description", "元描述"])
        evidence_text = issue.get('example_evidence', '') # Get evidence
        example_url = issue['examples'][0] if issue['examples'] else "http://example.com"

        if is_serp_issue:
            draw_serp_preview(slide, issue['title'], evidence_text, example_url)
            # Add other examples below SERP
            ex_title_box = slide.shapes.add_textbox(Inches(7), Inches(4.7), Inches(5.8), Inches(0.5))
            p = ex_title_box.text_frame.add_paragraph()
            p.text = "Other Affected URLs:"
            set_font(p.font, 12, True, RGBColor(100, 100, 100))
            
            ex_box = slide.shapes.add_textbox(Inches(7), Inches(5.0), Inches(5.8), Inches(1.0))
            tf = ex_box.text_frame
            for ex_url in issue['examples'][1:4]: # Skip first one as it's in SERP
                p = tf.add_paragraph()
                p.text = f"• {ex_url}"
                set_font(p.font, 10, False, RGBColor(0, 102, 204))
        else:
            # Right: Standard Examples List
        ...
        # Bottom: Suggestion
        sugg_bg = slide.shapes.add_shape(1, Inches(0.5), Inches(5.8), Inches(12.333), Inches(1.5))
        sugg_bg.fill.solid()
        sugg_bg.fill.fore_color.rgb = RGBColor(230, 244, 234)
        sugg_bg.line.color.rgb = RGBColor(40, 167, 69)
        
        sugg_box = slide.shapes.add_textbox(Inches(0.7), Inches(5.9), Inches(11.9), Inches(1.3))
...
if menu_key == "input":
    ...
    if st.session_state['audit_data'] is None:
        ...
        for i in raw_issues:
            ...
            if t not in grouped:
                grouped[t] = {
                    ...
                    "example_evidence": i.get("evidence", "") # Capture evidence
                }
            grouped[t]['count'] += 1
        ...
        with st.container(border=True):
            ...
            with c2:
                # Preview logic for SERP simulation
                is_serp_issue = any(k in s['title'] for k in ["Title", "标题", "Meta Description", "元描述"])
                if is_serp_issue:
                    ...
                    <div style="font-family: Arial, sans-serif; border: 1px solid #dfe1e5; border-radius: 8px; padding: 15px; background: white; box-shadow: 0 1px 6px rgba(32,33,36,0.28);">
                        ...
                        <div style="font-size: 14px; color: #4d5156; margin-top: 3px;">
                            Please provide a meta description to help search engines understand your page content...
                        </div>
                    </div>
